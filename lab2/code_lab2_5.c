...
int timeReceive;
int  QueueNum;
static char timeArr[10];
static char QueueMess;
...
SemaphoreHandle_t FDirSem = NULL;
SemaphoreHandle_t BDirSem = NULL;
   ...
   while (globalV == 0 )
	{
                    ...
                    xSemaphoreGive( BDirSem,1000 );

                        /* We have finished accessing the shared resource.  Release the
                        semaphore. */
                    xSemaphoreGive( semcount,1000);
     ...
     /* See if we can obtain the semaphore.  If the semaphore is not
      available wait 10 ticks to see if it becomes free. */
         xSemaphoreTake( semcount, 1000 );
                        ...
                        /* We were able to obtain the semaphore and can now access the
                        shared resource. */
        xQueueReceive(Queue,&timeReceive,10);
        itoa(timeReceive,timeArr);
        xQueueReset(Queue);
        SetDDRamAddr(0x40);                 //Sets the LCD cursor to the last row first position               
        while (BusyXLCD()){};
        putsXLCD(timeArr);       //String message
        while (BusyXLCD()){};
        SetDDRamAddr(0x48);
        while (BusyXLCD()){};
     //   QueueNum = uxQueueSpacesAvailable(Queue);
        itoa(QueueNum,QueueMess);
        while (BusyXLCD()){};
        putsXLCD(QueueMess);
        while (BusyXLCD()){};
        ...
        xSemaphoreGive( BDirSem, 1000 );
        xSemaphoreTake( semcount1, 1000 );//vTaskDelay(200); //Delay for 1 sec
        xSemaphoreTake( FDirSem, 1000 );
...
void appISR(void)
{
    ...
    timeSent =  xTaskGetTickCountFromISR();
    QueueNum = uxQueueSpacesAvailable(Queue);
    //itoa = (timeArrInt,timeArr);
    xQueueSendFromISR(Queue,&timeSent,NULL);
...
void main (void)
{
    ...
    Queue = xQueueCreate( 10, sizeof( unsigned long ) );
