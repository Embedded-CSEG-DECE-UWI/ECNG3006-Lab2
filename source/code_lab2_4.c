...
void TaskA(void *pdata)
{
   ...
   while (stopped)
	{        
       OSSemPend(LCD_SEMAPHORE,0,0);         //Waits on semaphore: LCD_SEMAPHORE to become available
...
extern void appISR()
{
   if(PIR1bits.TMR1IF) //Check if timer1 overflowed
   {
       ...
       OSSemPost(LCD_CSEMAPHORE);               //Release semaphore to signal to taskB
       OSSemPost(LCD_SEMAPHORE);
