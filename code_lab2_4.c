...
SemaphoreHandle_t FSema = NULL;
SemaphoreHandle_t BSema = NULL;
   ...
   //  const TickType_t xDelay = 1000 / portTICK_PERIOD_MS;
    ...
    {       
        ...
        available wait 10 ticks to see if it becomes free. */
         if( xSemaphoreTake( FSema, ( TickType_t ) 10) == pdTRUE )
...
void TaskB(void *pdata )
{
   ...
   
    {
        ...
        /* See if we can obtain the semaphore.  If the semaphore is not
        available wait 10 ticks to see if it becomes free. */
         if( xSemaphoreTake( BSema, ( TickType_t ) 10 ) == pdTRUE )
         { { xSemaphoreGive( FSema );
...
// Write the appropriate code to start FreeRTOS kernel
    ...
    xSemaphore = xSemaphoreCreateCounting( 1, 1 );
        BSema = xSemaphoreCreateCounting( 1, 1 );
            FSema = xSemaphoreCreateCounting( 1, 1 );
