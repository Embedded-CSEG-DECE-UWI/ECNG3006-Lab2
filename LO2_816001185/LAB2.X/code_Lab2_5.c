...
volatile int a =0  ;
volatile int TimeSend ;
volatile int TimeReceive ; 
...
//Check if timer0 overflowed
    ...
    TimeSend = xTaskGetTickCountFromISR() ; 
    xQueueSendFromISR(xMQueue, &TimeSend, NULL) ; 
...
void TaskB(void *pdata)
{
  ...
  while (a ==0) 
   ...
   //xSemaphoreTake(xSemaphoreCount,10);
      xQueueReceive(xMQueue,&TimeReceive,10)  ; 
      itoa(TimeReceive, Stime) ; 
    ...
    //vTaskDelay(50) ; 
      ...
      //  xSemaphoreTake(xSemaphoreTime, 1000);  
