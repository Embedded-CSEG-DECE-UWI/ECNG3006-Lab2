...
volatile int  Stimesend =  0 ;
volatile int Stimereceive;
volatile int unread;
volatile int Sunread;
volatile char timeget [14];
volatile char spaceget [14];
...
void appISR(void)
{
   ...
   Stimesend =  xTaskGetTickCountFromISR();
    ...
    xQueueSendFromISR( Queue1,&Stimesend,NULL );
   unread = uxQueueMessagesWaitingFromISR( Queue1 ); //
   xQueueSendFromISR( Queue1,&unread,NULL );
...
void TaskB(void *pdata )
{
    ...
    {
       // unread=uxQueueMessagesWaiting( Queue1 );
    
       xQueueReceive(Queue1, &Stimereceive, 10 );
        xQueueReceive(Queue1, &Sunread, 10 );
        
       itoa(Stimereceive , timeget);     
        itoa(Sunread , spaceget);
        xQueueReset(Queue1);
        while(BusyXLCD()) ; 
        WriteCmdXLCD(SHIFT_DISP_LEFT);
        while(BusyXLCD());
...
//    xTaskCreate(TaskA, "TaskA", configMINIMAL_STACK_SIZE, NULL, 1, NULL);
    xTaskCreate(TaskB, "TaskB", configMINIMAL_STACK_SIZE, NULL, 2, NULL); //not sure about what has to be done with FreeRTOSConfig.h
     Queue1 =  xQueueCreate( 10, sizeof(TickType_t ) );
    lcdconfig();
    INTCONbits.GIE=1;
