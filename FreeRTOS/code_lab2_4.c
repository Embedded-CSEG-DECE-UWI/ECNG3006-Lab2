   ...
   while (globalV == 0 )
	{
                    ...
                    xSemaphoreTake( FDirSem,100 );
                    xSemaphoreGive( BDirSem,100 );

                        /* We have finished accessing the shared resource.  Release the
                        semaphore. */
                    xSemaphoreGive( semcount,100);
	...
	{               
     /* See if we can obtain the semaphore.  If the semaphore is not
      available wait 10 ticks to see if it becomes free. */
        ...
        SetDDRamAddr(0x50);                 //Sets the LCD cursor to the last row first position               
        while (BusyXLCD()){};
        ...
        xSemaphoreGive( BDirSem, 100 );
        xSemaphoreTake( semcount1, 100 );//vTaskDelay(200); //Delay for 1 sec
        xSemaphoreTake( FDirSem, 100 );
